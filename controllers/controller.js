...
const logger = createLogger("controller filter:");
...
class Controller {
  ...
  static collectRequestParams(request) {
    ...
    if (request.openapi.schema.requestBody == null) {
      if (
        request.openapi.schema.parameters != null &&
  ...
  static async handleRequest(request, response, serviceOperation) {
    logger.info(`request -> ${JSON.stringify(request.openapi.schema)}`);
    logger.info(`request headers-> ${JSON.stringify(request.headers)}`);

    let obj = this.collectRequestParams(request);

    if (typeof obj[Object.keys(obj)[0]] !== "object")
      obj = { request: Object.assign({}, obj) };
    for (let prop of Object.keys(obj)) obj[prop].logger = response.locals.log;
    const serviceResponse = await serviceOperation(obj);
